...
class Dataset():
    def __init__(self, cf, cv_i=0, test_only=False, channel_last=True, h5_path=None, in_mem=True):
        '''
        cf: config.yml path
        ...
        self.config = cf
        self.h5_path = h5_path or os.path.abspath(self.config['data']['h5_path'])
        self.channel_last = channel_last
        if in_mem:
            with h5py.File(self.h5_path, 'r') as f:
